...
fun <T> repeatFlow(times : Int = -1, delay : Duration = 0.milliseconds, @BuilderInference block: suspend FlowCollector<T>.() -> Unit) =
    flow<T> {
        when {
            times < 0 -> {
                while (true) {
...
private abstract class ConnectableFlowImpl<T>(private val upStream: Flow<T>) :
    ConnectableFlow<T> {
...
private class SingleReplayConnectableFlow<T>(upStream: Flow<T>) : ConnectableFlowImpl<T>(upStream) {
    private var buffer = AtomicReference<Optional<T>>(
        None
    )
